...
* Loading `3bmd-ext-code-blocks.asd` adds support for github style fenced code blocks, with `colorize` support:
    ...
    Can optionally use `Pygments` instead of `colorize` by setting `3bmd-code-blocks:*renderer*` to  `:pygments`. ~~Lexer and formatter options (`-O`) can be specified like ```` ```c++|linenos=1````.~~ (Formatter options seem to allow XSS pretty easily, so options are disabled by default until some better solution is implemented. Interface may change when if it is reenabled.)
